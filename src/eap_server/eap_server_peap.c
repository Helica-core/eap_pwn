				    ...
				    struct wpabuf *in_buf)
{
	...
	wpa_printf(MSG_INFO, "EAP-PEAP: received Phase 2: code=%d "
				 ...
				 const struct wpabuf *respData)
{
	...
	wpa_printf(MSG_INFO, "EAP-PEAP: current state %d [%s]",
			data->state, eap_peap_state_txt(data->state));
			     ...
			     struct wpabuf *respData)
{
	...
	wpa_printf(MSG_INFO,
			"EAP-PEAP: get eap data from tls");
